...
my $ncands_ids = csv(
    in      => "$FindBin::Bin/../../eval/mass-ncands-ids.csv",
    headers => "auto"
);

my $afcars_ids = csv(
    in      => "$FindBin::Bin/../../eval/mass-afcars-ids.csv",
    headers => "auto"
);

# hashify by the QIC person ID
my %afcars_lookup = map { $_->{person_id} => $_ } @$afcars_ids;
my %ncands_lookup = map { $_->{person_id} => $_ } @$ncands_ids;
...
sub get_afcars_id {
    my $rec = shift;
    if ( exists $afcars_lookup{ $rec->{child_state_id} } ) {
        return $afcars_lookup{ $rec->{child_state_id} }->{rec_number};
    }
...
sub get_case_id {
    my $rec = shift;
    if ( exists $afcars_lookup{ $rec->{child_state_id} } ) {
        return $afcars_lookup{ $rec->{child_state_id} }->{case_id};
    }
    if ( exists $ncands_lookup{ $rec->{child_state_id} } ) {
        return $ncands_lookup{ $rec->{child_state_id} }->{case_id};
    }
    # warn "no AFCARS ID for QIC Person ID " . $rec->{child_state_id};
    $missing{case_id}++;
    return;
