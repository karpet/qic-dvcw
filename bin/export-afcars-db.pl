...
my $episodes_sth = $dbh->prepare("select * from episodes where child_id = ? order by RepDatYr asc, RepDatMo asc");
...
my $child_csv = Text::CSV_XS->new( { binary => 1, eol => $/, auto_diag => 1, } );
my $episode_csv = Text::CSV_XS->new( { binary => 1, eol => $/, auto_diag => 1, } );
open my $child_fh, ">:encoding(utf8)", $child_csv_file or die "$child_csv_file: $!";
open my $episode_fh, ">:encoding(utf8)", $episode_csv_file or die "$episode_csv_file: $!";
...
while (my $child = $children_sth->fetchrow_hashref()) {
    ...
    my $total_removals = $episodes->[0]->{TotalRem}; # baseline is first episode
    ...
    push @complete_episodes, $previous_episode; # last one is always most complete

    if ($removal_changes != scalar(@complete_episodes)) {
        die "Failed to capture $removal_changes complete episodes for child " . dump($child);
    }
    ...
    for my $f (qw( ClinDis MR VisHear PhyDis EmotDist OtherMed EverAdpt AgeAdopt )) {
        my $recent = $complete_episodes[-1]->{$f};
        if ($child->{$f} ne $recent) {
            # TODO
            $DEBUG and printf("Updating %s %s -> %s\n", $f, $child->{$f}, $recent);
            #$child->{$f} = $recent;
        }       
    }
